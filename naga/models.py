...
class Character(models.Model):
    ...
    headshot = models.ImageField(
        upload_to="naga/uploads/characterHeadshots/", blank=True, null=True, default="naga/blankUser.png"
    )
    ...
    def createDefaultSavingThrows(self):
        ...
        for name, fieldValue, parentStat, orderindex in startingSet:
            sthrow = SavingThrow(
                name=name, fieldValue=fieldValue, parentStat=parentStat, orderindex=orderindex, character=self
            )
            sthrow.save()
    ...
    def getAllSpells(
        self, includeSecondary=True, preparedOnly=False, secondaryPreparedOnly=False, known=True, showClassInName=False
    ):
        sps = self.getPrimarySpellClass().getSpells(
            known=known, preparedOnly=preparedOnly, showClassInName=showClassInName
        )
        for c in self.dndClasses.all():
            if c.isSpellcastingClass and not c.isPrimarySpellClass and includeSecondary:
                addSps = c.getSpells(
                    preparedOnly=(preparedOnly or secondaryPreparedOnly), known=known, showClassInName=showClassInName
                )
                newSps = {}
                for key in sps.keys():
